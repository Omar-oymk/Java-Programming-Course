...
class MyInteger
{
    ...
    // MATHS RULE:
    // *If a number n has a divisor greater than √n, then it must also have one smaller than √n.*
    // a better approach
    public boolean isPrime()
    {
        // cause anything less than that is not a prime number
        if(value <= 1) return false;
        for(int i = 2; i < Math.sqrt(value); i++)
            // if it is divisible directly that makes it a notPrime number
